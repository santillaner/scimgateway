 ...
 * @constructor
 */
const ScimGateway = function (bearer_map) {
  let server = null
  ...
  let config = scim_config.scimgateway
  ...
  const auth = async (ctx, next) => { // authentication/authorization
    ...
    const bearerToken = (ctx) => {
      return new Promise((resolve, reject) => { // bearer token
        if(bearer_map){
          ...
          if(result && result.length==2) tenant=result[1];
          if(tenant)
            var tenantToken=bearer_map[tenant];
          else reject(new Error("bad url: "+url))
          if (authType === 'Bearer' && !jwt.decode(authToken)) {
            if (tenantToken && tenantToken === authToken) resolve(true) // authentication OK
            else {
    ...
    try { // authenticate
      const arrResolve = await Promise.all([unauth(ctx), basic(ctx), bearerToken(ctx), bearerAzure(ctx), bearerJwt(ctx)]).catch((err) => {
        console.error(err);
      })
  ...
  router.get('/sync/ping', async (ctx) => { // auth not required
    const tx = 'hello'
  ...
  router.get(['/sync/(|scim/)(ServiceProviderConfigs|ServiceProviderConfig)',
    '/sync/:baseEntity/(|scim/)(ServiceProviderConfigs|ServiceProviderConfig)'], async (ctx) => {
  ...
  router.get(['/sync/(|scim/)Schemas', '/sync/:baseEntity/(|scim/)Schemas'], async (ctx) => {
  ...
  router.get(['/sync/(|scim/)Schemas/:id', '/sync/:baseEntity/(|scim/)Schemas/:id'], async (ctx) => { // e.g /Schemas/Users | Groups | ServiceProviderConfigs
  ...
  router.get(['/sync/(|scim/)(ResourceTypes|ResourceType)',
    '/sync/:baseEntity/(|scim/)(ResourceTypes|ResourceType)'], async (ctx) => { // ResourceTypes according to v2 specification
    const tx = scimDef.ResourceType
    ctx.body = tx
  })

  router.get([`/sync/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    `/sync/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
  ...
  router.get(['/sync/(|scim/)(Users|Groups|servicePlans)',
    '/sync/:baseEntity/(|scim/)(Users|Groups|servicePlans)'], async (ctx) => {
  ...
  //
  router.post([`/sync/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`,
    `/sync/:baseEntity/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`], async (ctx) => {
    ...
    try {
      ...
      const e = jsonErr(config.scim.version, pluginName, ctx.status, err)
      if(typeof err == "string" && err=='NoUser'){
        ctx.status = 404;
      }else if (err.name && err.name === 'DuplicateKeyError'){
        ctx.status = 409
      }else if(err.message && err.message === "User doest not exist on database"){
        ctx.status = 404
      }else{
        ctx.status = 500
      }
  ...
  //
  router.delete([`/sync/(|scim/)(!${undefined}|Users|Groups)/:id`,
    `/sync/:baseEntity/(|scim/)(!${undefined}|Users|Groups)/:id`], async (ctx) => {
    let u = ctx.originalUrl.substr(0, ctx.originalUrl.lastIndexOf('/'))
    u = u.substr(u.lastIndexOf('/') + 1) // u = Users, Groups
    const handle = handler[u]
  ...
  //
  router.patch([`/sync/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    `/sync/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    let u = ctx.originalUrl.substr(0, ctx.originalUrl.lastIndexOf('/'))
    u = u.substr(u.lastIndexOf('/') + 1) // u = Users, Groups
    const handle = handler[u]
  ...
  //          => Using same as patch
  // ==========================================
  router.put([`/sync/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    `/sync/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    let u = ctx.originalUrl.substr(0, ctx.originalUrl.lastIndexOf('/'))
    u = u.substr(u.lastIndexOf('/') + 1) // u = Users, Groups
    const handle = handler[u]
  ...
  //
  router.post(['/sync/api', '/sync/:baseEntity/api'], async (ctx) => {
  ...
  // {"eventName":"AsignAccessRoleEvent","subjectName":"RACF_System-B","userID":"peter01"}
  //
  router.put(['/sync/api/:id', '/sync/:baseEntity/api/:id'], async (ctx) => {
    const id = ctx.params.id
  ...
  // {"eventName":"AsignAccessRoleEvent","subjectName":"RACF_System-B","userID":"peter01"}
  //
  router.patch(['/sync/api/:id', '/sync/:baseEntity/api/:id'], async (ctx) => {
    const id = ctx.params.id
  ...
  //  GET = /api/{id}
  //
  router.get(['/sync/api', '/sync/api/:id',
    '/sync/:baseEntity/api', '/sync/:baseEntity/api/:id'], async (ctx) => {
  ...
  //  DELETE = /api/{id}
  //
  router.delete(['/sync/api/:id', '/sync/:baseEntity/api/:id'], async (ctx) => {
    const id = ctx.params.id
  ...
  const gracefulShutdown = function () {
    console.error("gracefulShutdown FUCK YOU", new Error())
    return;
  }
...
ScimGateway.prototype.getPassword = (pwEntity, configFile) => {
  return scim_config.scimgateway.auth.basic.password;
    ...
    if (value && value.constructor === String && value.includes(processEnv)) {
      ...
      try {
        ...
        try {
          ...
          if (err.name && err.name === 'processExtConfig') console.error( err)
          else {
        ...
        value = undefined
        if (err.name && err.name === 'processExtConfig') console.error( err)
        else console.error (new Error(`configuration failed - can't read external configuration file: ${err.message}`))
      }
      dotConfig[key] = value
    }
  }
